...
describe("isCeilingLight（UtilDeviceFW からの転記）", () => {
    ...
    it("LED_CEILING_MAT_* (0x04〜0x0A) は天井灯", () => {
        for (let c = 0x04; c <= 0x0a; c++) assert.equal(isCeilingLight(c), true, `0x${c.toString(16)}`);
    });
...
describe("その他の述語", () => {
    ...
    it("サイド RGB / スポットの製品コード", () => {
        for (const c of [0x18, 0x19, 0x56, 0x57]) assert.equal(hasSideRgb(c), true, `0x${c.toString(16)}`);
        for (const c of [0x16, 0x17, 0x54, 0x55]) assert.equal(hasSideSpot(c), true, `0x${c.toString(16)}`);
