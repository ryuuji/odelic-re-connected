 ...
 * ```
 *
 * ⭐ 純正アプリも天井灯でない器具には明るさコード 17/18/19（= 15%/10%/5%）を
...
export const MAIN_BRIGHTS = Array.from({ length: MAIN_STEPS }, (_, i) => (i + 1) * 5);
function clamp(v, lo, hi) {
    return Math.min(Math.max(v, lo), hi);
}
/** 器具が返す常夜灯の値（1〜3）→ `/night?level=` の値（0〜2）。C24-6 で反転する。 */
export function nightDeviceToLevel(deviceValue) {
    return clamp(3 - deviceValue, 0, 2);
}
/** `/night?level=` の値（0〜2）→ 器具が返す値（3〜1）。 */
export function nightLevelToDevice(level) {
 ...
 */
export function ladder(nightLight) {
    const rungs = [];
    if (nightLight) {
 ...
 */
export function rungIndexOfState(state, nightLight) {
    ...
    if (state.on !== true || state.bright === null || state.bright <= 0)
        return null;
