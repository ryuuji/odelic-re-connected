 ...
 *
 * ⭐ 根拠は純正アプリの `UtilDeviceFW` の述語**そのもの**（逆コンパイル結果から転記）。
 *    推測ではない。
 *
 ...
 * こちらが逆コンパイル結果の全体。
 */
const CEILING_LIGHT_CODES = new Set([
    // 単独比較
...
const ONLY_LIGHTNESS_CODES = new Set([0x8a, 0x91]);
/** `UtilDeviceFW.isInterface(int)` — 照明ではない。 */
const INTERFACE_CODES = new Set([0x1d, 0x88]);
/**
 * 照明ではない製品コード。
 *
 ...
 * 未知を理由に器具を隠すほうが害が大きい。
 */
export function capabilityOf(productCode, override = {}) {
    const applied = (cap) => {
        let out = cap;
