...
import { MAIN_BRIGHTS, MAIN_STEPS, NIGHT_STEPS, describeRung, ladder, nightDeviceToLevel, nightLevelToDevice, rungIndexOfState, } from "../src/ladder.js";
describe("主灯の段", () => {
    it("5% 刻みで 20 段（器具が受け付ける値だけ）", () => {
        ...
        for (const b of MAIN_BRIGHTS)
            assert.equal(b % 5, 0, `${b} が 5 の倍数でない`);
    });
...
describe("常夜灯の値の反転（C24-6）", () => {
    ...
    it("往復する", () => {
        for (const d of [1, 2, 3])
            assert.equal(nightLevelToDevice(nightDeviceToLevel(d)), d);
        for (const l of [0, 1, 2])
            assert.equal(nightDeviceToLevel(nightLevelToDevice(l)), l);
    });
});
describe("ladder()", () => {
