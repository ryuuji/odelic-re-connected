...
#   sudo ./install.sh <8桁ID> --with-backup   # 毎日 03:30 のバックアップも仕込む
#
# 8 桁 ID は純正アプリのメニュー画面に出ている番号（上位 4 桁 = HOMEID、
...
[ "$WITH_BACKUP" = 1 ] && NEEDED+=("$SRC/backup.sh")
for f in "${NEEDED[@]}"; do
    if [ ! -f "$f" ]; then
...
echo "  バックアップ   : $([ "$WITH_BACKUP" = 1 ] && echo '毎日 03:30' || echo '仕込まない')"
echo
...
if [ "$WITH_BACKUP" = 1 ]; then
    echo
    echo "################################################################"
    echo "#  バックアップ（Matter の fabric 鍵・器具の名簿・設定・CA の鍵）"
    echo "################################################################"
    "$SRC/backup.sh" --install
fi
...
if [ "$WITH_BACKUP" = 0 ]; then
    echo
    echo "⭐ バックアップを仕込んでいません。Matter の fabric 鍵と CA の鍵を失うと"
    echo "   全端末で登録をやり直すことになります:  sudo ./backup.sh --install"
fi
echo
