...
**解析の前提**: 公式アプリ v1.9.36 (versionCode 133) を jadx 1.5.6 で逆コンパイルした結果。
...
実際は GATT 接続で 1 台の器具と繋がり、そこを入口にしてメッシュ全体へコマンドを流す方式。
...
つまり一度パケットを落とすと、以降そのセッションの再組み立てが永久に失敗する。
...
`BLUETOOTH_ADVERTISE` が宣言されていることで、アドバタイズを使う（C3）ことが裏付けられる。
...
→ 起動時の 7 秒（症状 S1）は、この Ping 応答の収集待ちと考えられる **[推測]**。
...
#### C13-1. ⭐ 訂正: Ping には応答が返る（2026-08-04 実測）

自前実装の初期に「実機では Ping 応答が返ってこなかった」と記録していたが、**誤り**。
**平文で投げていたためだった。** 暗号化して送れば返る（C23-5 / `_auto_discover`）。

実機の器具 2 台とも `version`（`0x52C0 fw1.7`）が埋まっている。
これを埋めるのは Ping 応答の処理だけなので、この経路が生きている証拠になる。

| 経路 | 得られるもの |
| --- | --- |
| Ping（`0xFE` → `0xFF`） | MAC + vAddr + 機種コード + **ファームウェア版** |
| `0x02` → `0x80` | MAC + **製品コード**（Matter のデバイス種別に必須） |
| `0xD0 01` → `0xD7` | **グループ ID** |

⭐ 3 つとも必要なので、`_auto_discover` は Ping → `0x02` → `0xD0` の順に投げている。
⚠️ ただし **Ping 応答には器具でないものが混ざる**（→ C34-4）。
...
**最重要**。明るさ・色温度は素の 0-255 ではなく、製品タイプごとのルックアップテーブルから
...
**戻り値を捨てて 55/50 をハードコードしている**。公式アプリのバグまたは実装の残骸。
...
同じ役割の 3 点セット（サービス・書き込み・通知）を両方向で用意している構造。
...
当初「公式アプリが復号後の PDU を logcat に出している」と記述したが、**誤りだった。**
...
しかしリリースビルドでは `LogUtil` と `MeshLog` の全メソッドが空実装。
...
| 手段 | 内容 | 評価 |
| --- | --- | --- |
| Frida | `processData` / `sendEncry` の JNI 呼び出しをフックして引数・戻り値を覗く | ◎ 確実。公式アプリの動作を平文で観測できる |
...
調査の初期に型番として控えていた `PLTCEOC-05` が、**GATT の characteristic 値として
平文で読めた。**
...
公式アプリも `.so` も使わず、Raspberry Pi 3 から自前実装でメッシュに参加できた。
...
チャレンジは接続ごとに異なる乱数で、公式アプリは `.so` で応答を計算している
（C18-3 の実機ログでは応答値がチャレンジと無相関）。
しかし誤った応答を返すと拒否される一方、無応答なら通る。
...
公式アプリの実機ログ（C18-4）と **src の vAddr 以外は完全に同一**。

```
公式: 03 FF FF FF FF 20 09 00 00 00 C1 37 37 00 00 00 00 00 00 00
自作: 03 FF FF FF FF 20 25 00 00 00 C1 37 37 00 00 00 00 00 00 00
```

これで案 D（完全自前実装）が完全に実証された。
`libnative-lib.so`・Pairlink SDK・公式アプリのコードは一切不要。
...
公式アプリも `LE Set Random Address` でランダムアドレスを使っている（C18-7）ので、
自作アプリでもランダムアドレスにすべき。
...
電波上は `0x06`、復号後に `0x01` / `0x02` / `0x03` になる構造。
...
| 案 | 内容 | 難易度 |
| --- | --- | --- |
| F | Frida で Android の `processData` をフックし、暗号文と平文のペアを収集してアルゴリズムを特定 | 中 |
...
homeid/pwd から器具ごとの event key を導出して `event_key_list` に登録する。
...
公式アプリはプロビジョニング（ECDH、初回登録）で vaddr と鍵を確立し、
...
| 案 | 内容 | 評価 |
| --- | --- | --- |
| M | 公式アプリのローカル DB（`/data/data/.../databases`）から器具 vaddr と鍵を読む | 器具の vaddr が一発で判る。root か run-as が要る |
| N | 参加シーケンスを `.so` 上で完全再現（`processData` を JNIEnv 偽装で呼び、内部状態を構築してから `encry_data_handle`） | 大。JNIEnv エミュレーションが必要 |
| F | Frida で公式アプリの `processData` の in/out やランタイム鍵をダンプ | 確実。実機必要 |
...
- グループが 0 と 1 に分かれている実測と一致
  （→ [複数器具への対応](analysis/history.md) の裏付け）
...
⚠️ `SET_LINK` は「2 台目以降のバックアップリンク」にだけ返すものだった。
...
- `0x73`（`STATUS_LC615`）・`0x77`（`STATUS_RGB2`）を要求として投げても **無応答**。
  この 2 つは `MeshProfile` に定義があるだけで、**公式アプリも送っていない**
- 公式アプリが送る状態要求は `0x70` の 1 種類だけ（`ControllerAct.get_light_status`）
...
公式アプリはボタンを押すたびに **0 → 1 → 2 → 0** と巡回させている。
...
`OtaUpgrader_pairlink` はファイルを 20 バイトずつそのまま書き込むだけで
...
残っている未解読は「公式アプリも使っていないフィールド」だけなので、
...
Cypress（現 Infineon）CYW207xx 系の特徴。**Cortex-M4 なので Thumb-2**。
...
#### C27-2. 公式アプリの OTA UUID は「1 世代違い」だった
...
**公式アプリのどこからも参照されていない**（`grep` で APK 全体を確認）。
...
実験: Pi をメッシュに参加させたまま、**公式アプリ（Android）で操作** して
Pi が何を受信するか記録した。
...
- 公式アプリはグループ宛（`0xC1` / `0xC5`、末尾にグループ番号）を使っていた
...
公式アプリの実トラフィックで独立に裏付けられた。**

#### C28-3. 共存も確認できた（実験 C 相当）

公式アプリが参加・操作している間も **Pi のリンクは切れなかった**
...
→ 公式アプリより状態追従が正確になる。
公式アプリは他コントローラの操作を `syncAllState`（`0xC0` / チャネル 0x2A）でしか
拾わないが、実際に飛んでいるのは `0xC1` / `0xC5`（グループ宛）なので 取りこぼす。

### C29. ⭐⭐ 主リンク／バックアップリンクのライフサイクル（2026-07-25）

`PlMeshPeripheral` を読み切って、公式アプリの接続管理を完全に把握した。
C23-6 で判った「`SET_LINK` はバックアップ専用」の全体像がこれで繋がる。

#### C29-1. 接続してきた器具は全部いったんバックアップになる
...
#### C29-3. ⭐ 公式アプリは主リンク 1 本だけを残し、他を追い出す
...
| 5 | 送信元を区別していない（`MeshCommon` はシングルトン） | 器具が 2 台以上あると別々の転送が同じバッファに混ざる |
...
#### C31-1. 公式アプリの AD の 6 バイトは端末の MAC ではない
...
→ 独立させた（`--ctrl-id-file` に永続化。公式アプリと同じ方針）。
...
| 2 | 新規 | 既知 | 残っていた | ✅ 広告開始の 4.9 秒後に接続 |
| 3 | 既知 | 新規 | 切断済み | ✅ 広告開始の 5.2 秒後に接続 |
...
公式アプリの広告は **拡張アドバタイズのセット #2** として出ていた。
...
#### C31-4. ✅ Android のランダムアドレス問題は存在しなかった
...
公式アプリも参加のたびに `stopAdvertise(); sleep(100); startIBeaconAdvertise(...)` を
...
「公式アプリは参加後に広告を止める」を採用していなかった）。実測すると:
...
| スライダー模擬（143 ms 間隔 × 16 通） | 最終値に両器具が収束・取り違え 0・古い操作 15 件を破棄 |
...
なお公式アプリが使う `01 15 55`（exit_cmd）は **BlueZ では使えない**。
...
### C34. ⚠️⚠️ 同じ器具が 2 つの vAddr で見える（2026-08-03）

**症状**: Matter（Google Home）で器具が「反応なし・状態不明」になる。
利用者からは「別のリモコンが接続しているとエラーが起きる」として報告された
（⚠️ 発生条件は C34-4 のとおり別にありそう）。

`GET /metrics` の到達率に **同じ MAC が 2 行** 並んでいた。

| 器具 | 到達率 | 通電 |
| --- | --- | --- |
| `EC:C5:7F:81:DE:CD` | 100%（30447 回） | 応答あり |
| `EC:C5:7F:81:DE:CD` | **0%（30445 回）** | 応答なし |
| `EC:C5:7F:80:28:A6` | 100%（2694 回） | 応答あり |

到達率のキーは vAddr なので、**1 台の器具が 2 つの vAddr で登録されていた**。
回数が 2 回しか違わないので幽霊は参加直後にできていて、
以後 3 万回の状態要求に **一度も応答していない**。

#### C34-1. なぜ「反応なし」になるのか

`odelicd` は一度見つけた器具を消さないので、応答しない側が幽霊として残り続ける。

その幽霊は 3 回連続で取りこぼした時点で `absent` になる。ところが `absent` は
**「通電が切れた器具」を見分ける唯一の手段**として Matter ブリッジと設定ページが見ている。
両者は器具の同一性を MAC で取っているので、同じ MAC の 2 行のうち幽霊が後に来ると

- Matter: `Reachable = false` → Google Home から「反応なし」・状態が「不明」
- 設定ページ: 同じ照明のカードが 2 枚、片方が「通電なし」

**生きて 100% 応答している照明が操作できないように見える。**

#### C34-4. ⭐⭐ 幽霊の正体は **自分の vAddr** だった（実機で確定）

`GET /info` を採ったら一発で割れた。

```
own_vaddr = 05 00 00 00 / device_num = 2 / devices_found = 3
  key=01000000  mac=EC:C5:7F:81:DE:CD  ver='0x52C0 fw1.7'  ← 本物（主リンクの器具）
  key=05000000  mac=EC:C5:7F:81:DE:CD  ver='0x52C0 fw1.7'  ← 幽霊
  key=09000000  mac=EC:C5:7F:80:28:A6  ver='0x52C0 fw1.7'  ← 本物
```

**幽霊の vAddr は `own_vaddr` と同じ**。つまり登録されていたのは **自分** だった。
`device_num`（器具が申告した台数）は 2 なのに `devices_found` が 3 になっている。

⭐ さらに **ファームウェアのバージョンが入っている**ことが決定的。
`version` を埋めるのは Ping 応答（チャネル `0xFF`）の処理だけなので、
この 3 件はすべて **Ping 応答から作られた**。
つまり器具は

> **主リンクの器具の MAC と、その器具が自分に割り当てた vAddr の組**

を Ping 応答として返している（C23-4 のレイアウトでは `[6..11]` が MAC、`[12..15]` が vAddr）。
自分は器具ではないので、この組で器具を作ってはいけない。

⚠️ したがって「他のリモコンが繋がると起きる」という当初の見立ては
**発生条件としては外れている可能性がある**（`own_vaddr` はリンクごとに割り当てられるので、
繋ぎ直しで値が変わると幽霊も増える）。ただし症状と因果の鎖は同じで、対策も変わらない。

⚠️ 素性は 2 通り考えられ、まだ確定していない。

| 可能性 | 根拠 |
| --- | --- |
| 連結先の器具が自分（コントローラ）を 1 ノードとして返している | MAC が主リンクの器具と一致する |
| **自分と同じ vAddr を割り当てられた別のコントローラ**（＝ 別のリモコン） | 利用者の報告条件と一致する |

⚠️ どちらでもありうる。自分の vAddr は **参加のたびに変わる**
（実測: `05 00 00 00` → 2 回の再参加を挟んで `19 00 00 00`）ので、
「器具が固定の枠を返している」とは言い切れない。
⭐ 次に起きたときは `own_vaddr` の**その時点の値**で登録されるはずで、
`#M own_vaddr_claim` が時刻とともに残る。それで切り分けられる。

→ 対策: **自分の vAddr を名乗る応答では何も作らない。**
⚠️ MAC が既知でも **別名として束ねない**（当初は束ねる実装にしたが撤回した）。
素性が不明なフレームを器具の応答として到達率に数えると、
**届いていないものを届いたと言うことになる**（P4 違反）。
本物の器具は自分の vAddr で 100% 応答するので、落として失うものはない。
`#M own_vaddr_claim` と `recv.own_vaddr_claim` で回数を数えている。

#### C34-5. ⭐⭐ 一般解: 一度も状態要求に答えない登録は器具ではない

C34-4 の対策は「この出所」を塞いだだけで、**別の出所から幽霊が来たら同じ症状になる**。
しかも MAC が本物と違う幽霊なら MAC で畳めないので、
**Matter に「存在しない照明」が 1 台増える**（今回より悪い）。

そこで出所に依存しない判定を入れた。根拠は 1 行で言える。

> ⭐ **発見は応答によってしか起きない。**
> だから本物の器具は、発見された時点で少なくとも 1 回は電波を返している。
> それなのに状態要求（`0x70`）に一度も答えないものは、器具ではない。

| 状態 | 判定 | 扱い |
| --- | --- | --- |
| 一度答えた → その後沈黙 | ⭐ 壁スイッチで通電が切れた器具 | 残す。`absent` で `Reachable = false`（M6-4 のまま） |
| **一度も答えたことがない** | ⚠️ 器具ではない | 6 回連続で取りこぼしたら一覧から外す |

⚠️ 判定材料は `status_replies`（`0x71` / `0x35` を受けた回数）。
`state_updated_at` では代われない — **他コントローラの操作を観測しただけでも更新される**ため。

⚠️ 外すのは「**他の器具が答えた**要求で取りこぼしたとき」だけ。
全員無応答（リンクが死んだ）ときに外すと一覧が消える。

⚠️ 閾値 6 回は `absent`（3 回）より大きくして、通電が切れただけの器具を巻き込まない。
外しても実害はない — 本物なら次の探索で戻ってくる。

あわせて 2 つの安全網を入れた。

- **Ping 応答の妥当性検査**: MAC がオール 0 / vAddr がオール 0・ブロードキャスト・
  自分と同じなら採らない。捨てた数は `recv.ping_rejected`
- **台数の不変条件**: 参加時に器具が申告した台数（`device_num`）を登録数が超えたら
  `#M device_count_over` を出す。⭐ **今回はこの 1 行で 7 月に気づけたはずだった**
  （`devices_found` 3 / `device_num` 2）。⚠️ 後から器具が増えることもあるので警告に留める

#### C34-2. 直したこと

⭐ **器具の同一性は MAC。vAddr は宛先にすぎない。** これを 3 か所で徹底した。

| 場所 | 直したこと |
| --- | --- |
| `odelicd` | ⭐ **自分の vAddr を名乗る応答では何も作らない**（C34-4） |
| `odelicd` | ⭐⭐ **一度も状態要求に答えない登録は一覧から外す**（C34-5・出所に依存しない） |
| `odelicd` | Ping 応答の妥当性検査と、申告台数を超えたときの警告（C34-5） |
| `odelicd` | 登録は MAC ごとに 1 台。新しい vAddr は**別名として束ねる**（`vaddr_alias`） |
| `odelicd` | 別名で返ってきた応答も同じ 1 台として数える（到達率が割れない） |
| `odelicd` | 個別送信の宛先は**応答が返った vAddr**（`reply_vaddr`）を優先する |
| `odelicd` | 幽霊が汚した到達率は捨てる（`/metrics` に 0% の行を残さない） |
| `@odelic/common` | `foldDevicesByMac()` — 受け取る側でも MAC ごとに畳む |
| `odelic-matter` / `odelic-web` | 畳んでから使う。**応答している方**を残す |

⚠️ どちらの vAddr が正しいかは、送ってみるまで判らない。
`absent` でない方＝**実際に応答している方**を正とするのが唯一の確実な判定。

#### C34-3. 診断

```bash
# ⭐ own_vaddr と devices[].vaddr が一致していないかを見る（これが C34-4 の signature）
curl -s localhost:8080/info \
  | jq '{own_vaddr, device_num, devices_found, vaddr_alias,
         devices: [.devices[] | {mac, vaddr, reply_vaddr, status_replies}]}'
curl -s localhost:8080/metrics | jq '.recv | {ping_rejected, own_vaddr_claim}'
sudo journalctl -u odelicd \
  | grep -E '#M (vaddr_alias|vaddr_reply|own_vaddr_claim|phantom_device|device_count_over)'
```

⭐ `device_num`（器具の申告）と `devices_found`（登録数）がずれていたら疑う。
⭐ `status_replies` が 0 の器具は「発見はされたが一度も答えていない」= 器具ではない疑い。
`vaddr_alias` が空でなければ、同じ器具を別の vAddr でも見ている。

⚠️ 残る疑問: この Ping 応答を **何が引き起こしているのか**（起動時の 1 回か、
リンクの張り直しか、他のコントローラの参加か）はまだ切り分けていない。
`#M own_vaddr_claim` の時刻と `#M link_up` / `join` の時刻を突き合わせれば判る。
`POST /verbose?on=1` で受信 PDU を全部出せば、どのチャネルで来るかも確定できる。

---

### C35. ⚠️⚠️ 届いた応答を「取りこぼし」にしていた（2026-08-04）

**症状**: 別のリモコンを**追加している最中**に、Matter（Google Home）が照明を
「反応なし」と報告する。照明自体は正常に動いている。

**照明は一度も壊れていない。締め切りを 162 ms 早く切っていただけだった。**

#### C35-1. ⭐⭐ 実機のログ（1/1000 秒まで並べると一目で分かる）

```
[15166.523] #M miss vaddr=09000000      ← 窓が閉じて「取りこぼし」確定
[15166.524] #M miss vaddr=01000000      ← 2 台とも
[15166.524] #M no_response streak=1 expect=2
[15166.686] 状態更新 EC:C5:7F:80:28:A6 …  ← ★ 162 ms 後に応答が到着（届いていた）
[15166.702] 状態更新 …                     ← 計 6 件（resend=3 × 2 台）
[15167.088] #M miss vaddr=09000000  streak=2   ← 追い打ちの窓も閉じてから
[15167.108] #M rtt vaddr=09000000 ms=18.1     ← 直後に応答（前の要求の遅延応答を誤対応）
[15168.033] #M miss vaddr=09000000            ← 3 連続 → absent
```

ブリッジ側:

```
14:44:13  奥の照明: Reachable = false     ← Google Home が「反応なし」
14:44:34  奥の照明: Reachable = true      ← 21 秒後に自然復帰
```

状態更新の中身（`on=True bright=20`）は正常。器具は答えていて、値も正しい。

⚠️ 通常の RTT は p50 43〜60 ms / p99 90〜117 ms（実測 max 153 ms）。
リモコン追加中だけ **500 ms を超えた**。`recv.status_unsolicited` が
その指紋（＝進行中の要求に対応づけられなかった応答）。

#### C35-2. 原因は 4 つ重なっていた

| # | 原因 | 証拠 |
| --- | --- | --- |
| 1 | **届いた応答を捨てる。** 窓（`probe_window_ms` 下限 500 ms）が閉じた後の応答は数えず、`miss` が確定する | `recv.status_unsolicited = 40` |
| 2 | **全員無応答でも器具個別に `absent`（= 通電が切れた）を立てる。** 原因はメッシュ側なのに器具の障害として Matter に報告する | `no_response … expect=2` の直後に `absent` |
| 3 | **`resend=3` が混雑の元。** コードと docs（C33-5）の結論は 1 なのに `odelicd/install.sh` が 3 を書き込んでいた | `status_dup = 4901` / probe 1237 回（1 回あたり 4 件の重複） |
| 4 | **ブリッジの追い打ちが自己増幅する。** in-flight 1 本制限のため**追い打ちが前の窓を閉じ**、3 連続 miss が 1.5 秒で完成する | 上のログの 15167 / 15168 の並び |

⚠️ 4 は皮肉な副作用。「通電切れの検知を 3 分から 32 秒に縮める」ための仕掛け（M6-4）が、
遅延スパイクでは**偽の「反応なし」を数秒で作る装置**になっていた。

#### C35-3. 直したこと

| | 内容 |
| --- | --- |
| A ⭐⭐ | **遅延応答の猶予**（`probe_grace_ms` = 500〜1500 ms）。窓が閉じても確定させず「締めかけ」として残し、遅れて届いた応答を拾う。⭐ 遅い RTT も計測に入れるので、**p90 が上がって窓が自動で広がる**（負のフィードバック） |
| B ⭐ | **`absent` は「他の器具が答えた要求」でのみ立てる**（`note_delivery(blame=…)`）。全員無応答は `dead_link` の担当。到達率（EWMA）には正直に入れる |
| C ⭐ | **`resend` の既定を 1 に統一**（installer 3 → 1）。混雑を 1/3 に |
| D | **`miss` イベントに `answered`（応答した器具数）を載せ**、ブリッジは `answered=0` では追い打ちしない |

⭐ 応答の対応づけは **FIFO**（まだ待っている要求のうち古い方に数える）。
器具は投げた順に答えるので、これが正しい相手であり、
「窓が閉じた直後に届いた応答」が取りこぼしを取り消せる。

⚠️ 器具 1 台だけの環境では「全員無応答」と「その 1 台が無応答」が区別できない。
ただし 1 台が落ちれば GATT リンクも切れて `connected = false` になるので、
ブリッジは別経路で「状態不明」にできる。

#### C35-4. 診断

```bash
curl -s localhost:8080/info | jq '.tuning'   # resend / probe_window_ms / probe_grace_ms
curl -s localhost:8080/metrics | jq '.recv, .probe'
#   status_late        … 猶予で拾えた応答（これが増えるのは正常。捨てずに数えている）
#   status_unsolicited … 猶予でも拾えなかった応答（増えるなら猶予が足りない）
#   status_dup         … 自分の連射に対する重複（resend=1 なら 0 に近づく）
sudo journalctl -u odelicd | grep -E '#M (miss|no_response|dead_link)'
```
...
C18-3 で PDU が平文だと判明し、C19 で Raspberry Pi から自前実装でメッシュ参加に成功した。
...
- 期待状態まで送り続ける（P2）→ **公式アプリは 1 回しか送らない**（C18-5 で実証）
- 冪等な操作（P3）→ 絶対値指定なので そのまま冪等
- 確認できるまで成功と言わない（P4）→ 状態イベントで照合
- 待たせない UI（P5）→ 完全に自作範囲
- 位置情報権限の除去（C11）

⚠️ ライセンス: 案 A・C は公式アプリのバイナリ／コードを利用するため
...
- [ ] `01 18 xx 00`（CMD 0x18 = 24）の意味。公式アプリも「Unknow CMD」として捨てている
      → 実測値は 0x0C / 0x14 / 0x17。参加時に器具が送ってくる
- [ ] `0x71` 応答の `data[8..12]` の意味（実測は全て 0。サイド RGB の枠らしい）。
      `syncState()` は読み捨てているので手がかりがない。
      ⭐ 手前の `data[7]` は ナイトライトの状態 だと判っている（C24-5）
- [ ] `Check Data Route[0x07]`（中継経路の取得）が電波側にも露出しているか
      → 露出していれば メッシュのトポロジ診断 に使える（C26-3）
...
- [ ] `assets/ota/*` のフォーマットと、ファイル名の数値の意味
      → 暗号化はされていないが独自コンテナで圧縮（または差分パッチ）されており、
      そのままでは逆アセンブルできない。手元の器具（21184）向けは同梱されていない。
      チップは Cypress CYW207xx 系（Cortex-M4）（C25 / C26-5）
