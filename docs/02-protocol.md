...
| 項目 | 変換 | 例（ID = `99833900`） |
| --- | --- | --- |
| **HOMEID** | 10 進数として `parseInt` → **リトルエンディアン 4 バイト** | `"9983"` → 9983 = 0x26FF → `FF 26 00 00` |
| **パスワード** | **ASCII 文字コードそのまま** | `"3900"` → `33 39 30 30` |
...
| --- | --- | --- | --- | --- |
| **`99833900`** | **`9983`** (0x26FF) | **`FF 26 00 00`** | **`3900`** | **`33 39 30 30`** |
...
旧 ID `88039728`（HOMEID `8803` / パスワード `9728`）は
ID 変更・再ペアリングにより**無効**。上記の分割規則を 2 例で裏付ける材料としてのみ意味を持つ。

→ **HCI ログで HOMEID を探すときは `FF 26` を検索する。**
...
AD Type 0xFF (Manufacturer Specific Data)
  ...
  [3]     FF
  [4]     05           ADV_PHONE
  [5..8]  FF 26 00 00  HOMEID（現在の ID 99833900 → 9983 のリトルエンディアン）
...
```powershell
python tools/btsnoop.py find artifacts/btsnoop_hci.log C0FF05FF260000
```
...
| C16: HOMEID | `FF 26 00 00` | ✅ **平文で確認** |
| C16: パスワード | `33 39 30 30`（"3900"） | ✅ **平文で確認** |
| C3: ADV_PHONE | `C0 FF 05` + HOMEID | ✅ HOMEID = **9983** をデコード |
...
# スマホ → 器具（応答）★ HOMEID + パスワード "3900" が平文
...
C16 の結論（HOMEID = `FF 26 00 00` / パスワード = `33 39 30 30`）が電波上で完全に一致した。
...
器具 → Pi:  01 19 34 AE E3 B5 A9 8C F2 E6 58 82 8D FF 61 62 A9 89
        ...
        ↓
器具 → Pi:  01 00 FF 26 00 00                       GET_PASSWORD（HOMEID を提示）
Pi → 器具:  02 00 FF 26 00 00 33 39 30 30           ★ 平文パスワードで応答
        ↓
...
hcitool -i hci0 cmd 0x08 0x0008 14 02 01 06 10 ff 00 00 c0 ff 05 ff 26 00 00 <MAC 6 バイト> 00...
...
```
鍵1 = FF 33 26 39 00 30 00 30  4C 4F 47 49 4E 4B 45 59   ("...LOGINKEY")
鍵2 = FF 33 26 39 00 30 00 30  45 56 45 4E 54 4B 45 59   ("...EVENTKEY")
...
<< 01 19 F3 37 07 C9 9D ED 00 16 56 17 8F A2 D7 52 03 8E
   復号 → FF 26 00 00 | F6 C2 B4 8D | 08 08 08 08 08 08 08 08
...
拡張ADV set#2 データ(20B): 02 01 02 10 FF 00 00 C0 FF 05 FF 26 00 00 19 7D AB 00 EE 05
