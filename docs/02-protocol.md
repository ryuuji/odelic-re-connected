...
→ 対策: **自分の vAddr を名乗る応答では器具を作らない。**
既知の MAC なら、その 1 台の別名として束ねる（応答を取りこぼさないため）。
...
| 場所 | 直したこと |
| --- | --- |
| `odelicd` | ⭐ **自分の vAddr を名乗る応答では器具を作らない**（C34-4） |
...
curl -s localhost:8080/info \
  | jq '{own_vaddr, device_num, devices_found, vaddr_alias, devices: [.devices[] | {mac, vaddr, reply_vaddr}]}'
sudo journalctl -u odelicd | grep -E '#M (vaddr_alias|vaddr_reply|own_vaddr_claim)'
```
