...
**症状**: 別のリモコンがメッシュに繋がっていると、器具が「反応なし」になる。
...
回数が 2 回しか違わないので、片方は参加直後の探索（`0x02` / `0xD0` のブロードキャスト）で
できていて、以後 3 万回の状態要求に **一度も応答していない**。

#### C34-1. なぜ「反応なし」になるのか

vAddr は器具の固定 ID ではない（実機で `01` → `05` に変わるのを既に観測している。
docs/07-matter.md §M6）。他のコントローラが参加するとメッシュの構成が変わるため、
探索応答が **別の vAddr を名乗って複数届く**。`odelicd` は一度見つけた器具を消さないので、
応答しない側が幽霊として残り続ける。
...
```bash
curl -s localhost:8080/info | jq '.vaddr_alias, [.devices[] | {mac, vaddr, reply_vaddr}]'
sudo journalctl -u odelicd | grep -E '#M vaddr_(alias|reply)'
```

`vaddr_alias` が空でなければ、この現象が起きている。
別のリモコンの接続と時刻が対応するかを `#M` 行で確認できる。

⚠️ 残る疑問: 幽霊の vAddr を名乗る応答が **どの経路で来ているのか**は未解明。
`_handle_data_event` はチャネル（`data[5]`）を `0xFF`（Ping 応答）以外は区別せずに
同じレイアウトで読んでいるので、リモコン系チャネル（`0x11` / `0x12`）の
フレームを誤読している可能性もある。`POST /verbose?on=1` で受信 PDU を全部出して
チャネルの分布を見るのが次の一手。
