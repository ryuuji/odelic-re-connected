...
HCI スヌープログは Android 端末の内部に溜まるため、記録時に PC は不要。
...
「公式アプリが実際に何を送っているか」という 最も価値の高い正解データ が取れる。
...
GATT で流れる PDU は暗号化されており、**公式アプリのログ出力は
...
| 確認 | フィルタ | 判定 |
| --- | --- | --- |
| GATT 接続が発生しているか | `bthci_evt.code == 0x3e`（LE Meta Event） | 接続完了イベントが無ければコネクションレス確定 |
...
Wireshark では手作業になる差分解析を機械的にやるためのもの。
