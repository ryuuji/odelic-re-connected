...
### ⚠️⚠️ install.sh はテストの失敗で止まらない

以前は `npm test` を `set -e` の下でそのまま呼んでいた。**これは誤りだった。**

Pi 3 は遅いので時間に依存するテストが落ちることがある。そこで中断すると
スクリプトの残り（特権ヘルパの設置・sudoers・パスワードの生成）が実行されず、
「画面は新しいのに sudo が通らない」という分かりにくい状態が残る。2 度踏んだ。

→ ⭐ **テストは「気付くため」に走らせる。配備を止める門にはしない。**
落ちたら警告を出し、インストールの最後にもう一度言う
（途中の出力は npm のログに流れて見えないため）。

⚠️ 開発機では止めたい。`ODELIC_STRICT_TEST=1` を付けると中断する。

```bash
sudo ODELIC_STRICT_TEST=1 ./install.sh <8桁ID>
```
