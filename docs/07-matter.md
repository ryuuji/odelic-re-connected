...
⚠️⚠️ **全器具が同時に無応答なら追い打ちしない**（C35-2）。
誰も答えていないなら原因は器具ではなく入口（メッシュ）で、追い打ちは無駄なうえ
**混雑を増やして誤判定を自分で作る**。実機では、リモコン追加中の
数百 ms の遅延がこの仕掛けで増幅され、**生きている照明が 21 秒間
「反応なし」になった**。判定材料は `miss` イベントの `answered`（応答した器具数）。
入口の復旧は odelicd の `dead_link`（リンクの張り直し）に任せる。
