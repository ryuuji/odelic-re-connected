...
| シーン / グループ設定変更 / タイマー | — | ❌ **スコープ外**（PLAN.md の破壊的操作） |
...
**[事実]** 天井灯でない器具（`isCeilingLight() == false`）に対して、純正アプリは
`setlight(vAddr, 0, level + 17, 0)` = **明るさコード 17/18/19（= 15% / 10% / 5%）**を
常夜灯の代用として送る（C24）。

→ つまり純正アプリも「常夜灯は明るさ軸の下端」と扱っている。
...
- OnOff の `On` は `POST /on`。⭐ 器具が記憶していた明るさに戻す（`37 37`）ので、
  **Matter の「On は前の明るさに戻す」と意味が完全に一致する**。
  戻った実値は直後の `0x71` 応答で分かるので `CurrentLevel` に反映する
...
- ⭐ C33-5「1 操作 = 1 通で足りる」（到達率 0.993〜1.000）と、
  C28 で観測した純正アプリの「143 ms 間隔で 16 連射」という反面教師に沿う
...
| | |
| --- | --- |
| 同一性 | ⭐ **器具の MAC**。⚠️ vAddr は変わる（実機で `01` → `05` に変わった）ので使わない |
...
**[事実]** `odelicd` は器具一覧を**メモリにしか持っていない**
...
| 起動時 | ⭐ **名簿からエンドポイントを先に復元**し、`Reachable = false` で出す |
...
**[事実]** `odelicd` は一度見つけた器具を `devices` から**削除しない**。
...
⚠️ **検知には状態要求が必要**なので `statusRefreshSec > 0`（既定 60 秒）が前提。
3 回の取りこぼしで判定するため、**検知までおよそ 3 分**かかる。
`statusRefreshSec = 0` にすると、操作したときしか更新されない。
...
なお **`?wait=1&timeout=1500` は使っている**ので、成功した場合は
...
- `odelicd` は自分のコマンド後と、**他コントローラの操作を観測したとき**に自動で
  状態を確認する（`_schedule_confirm`）。純正アプリからの操作も 1 秒以内に Matter へ届く（C28）
...
| # | 症状 | 原因と対処 |
| --- | --- | --- |
| 1 | エンドポイントの初期化が失敗する | `colorControl.colorMode` / `enhancedColorMode` は**必須属性**。設定しないと Conformance `M` で落ちる |
| 2 | 本物のコントローラ操作が無視される | エコー抑止の基準値を「パッチを当てたときだけ」更新していたので古くなっていた。→ **常に更新する** |
| 3 | 送信が二重になる・順序が入れ替わる | `setInterval(() => void poll())` で **await していなかった**ためポーリングが重なる。→ **自己再スケジュール型にして 1 周ずつ直列に回す** |
| 4 | ⭐⭐ **古い値がコマンドとして送られる** | ポーリングの書き戻しが、まだ送信していないユーザー指示を上書きしていた（Google Home で 80% にした 50 ms 後にポーリングが 30% へ引き戻し、その 30% が送られる） |
| 5 | ⭐ **Pi 実機でだけ古い値が送られる** | ユーザー操作より前に始まった書き戻しが遅れて着弾し、それが**新しいユーザー操作として再解釈**されていた。→ 世代番号（`intentSeq`）で、計算中に操作が入った書き戻しを**捨てる** |
| 6 | ⭐⭐ **色温度だけ 1 つ前の値が送られる** | `applyFromDevice` の先頭で `colorPercent` を**pin の判定より前に無条件で器具の現在値に上書き**していた。他のフィールドは守ったのにこの 1 行だけ漏れていた。→ `!isPinned("mireds")` を条件に加える |
...
⚠️ #5 は**開発機（速いマシン）では一度も再現せず、Pi 3 で初めて出た。**
...
**「器具の状態としてこちらが書いた値」と一致するなら送らない**という値比較だけで止める。
...
4. ⚠️ 失敗時
   ...
   - 回避経路: `chip-tool` / Apple Home / Home Assistant で先に commissioning し、
     **multi-admin で Google Home に共有**する
...
**`/var/lib/odelic-matter` を消して未 commissioning に戻してから再登録**した。
...
| 「消して」 | 消灯 | `on=false` |
| 純正アプリで操作 | Google Home が 1〜2 秒で追従 | C28 の観測経路が Matter まで通る |
