...
引き継げないのは**器具の表示名と配置だけ**（公式アプリのローカル DB にしかない）。

## ️秘密情報の扱い

- **8桁IDの下位4桁は実態としてメッシュのパスワードです。**
- 知られると近隣の誰でもその照明を操作できるようになります。
- リポジトリに出てくる `12345678`（HOMEID `1234` / パスワード `5678`）はすべてプレースホルダです。
- 実値は `install.sh` の引数で渡し、**`/etc/default/odelicd`（0600 root）** に保存されます
