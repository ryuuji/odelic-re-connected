...
| 同上（設定ページからは操作できる） | ⭐ Pi の IPv6 が消えています。→ [音声操作だけ効かなくなったとき](#音声操作だけ効かなくなったとき) |
...
| ⭐ 音声操作だけ効かない（設定ページからは動く） | Pi の IPv6 が消えています。→ [音声操作だけ効かなくなったとき](#音声操作だけ効かなくなったとき) |
...
設定ページからは操作できるのに、Google Home / Apple Home / Alexa からだけ効かない。
**アプリ上は成功したように見えるのに照明が変わらない。** この形なら、まず IPv6 を疑ってください。

```bash
ip -6 addr show wlan0     # 有線なら eth0
```

`inet6` の行が 1 つも無ければこれです。

⭐ ハブは Matter の通信を **IPv6 でしかしません。** 一方 `odelicd` は Bluetooth、
設定ページは IPv4 なので無傷のまま残ります。だから「音声操作だけ死ぬ」という形になります。
ログにもエラーは出ません。**操作が届かなくなるだけ**です
（設定ページ →「ログ」タブで、操作しても何も増えないことで分かります）。

戻し方は 3 手です。

```bash
# 1. すぐ戻す（⭐ Wi-Fi は切れません）
echo 0 | sudo tee /proc/sys/net/ipv6/conf/wlan0/disable_ipv6

# 2. ブリッジに気付かせる（起動時にしか見に行かないので再起動が要ります）
sudo systemctl restart odelic-matter

# 3. 再発を止める（接続名は `nmcli con show` で確認）
sudo nmcli con mod "<接続名>" ipv6.method auto
```

⚠️ **3 をやらないと、次に Wi-Fi が切れて再接続した瞬間にまた消えます。**
Wi-Fi の設定に IPv6 が書かれていないと、NetworkManager が IPv6 ごと止めてしまうためです。

⚠️⚠️ **設定を変えた直後は気付けません。** その時点で付いているアドレスはそのまま残るので、
何時間も正常に動き続けます。**再接続して初めて牙を剥きます。**
Wi-Fi の設定をいじった記憶があるなら、切れるのを待たずに 3 を確かめてください。

直ったかどうかは、ブリッジがハブとつながり直したかで分かります。

```bash
sudo journalctl -u odelic-matter -n 30 | grep -E 'Resumed session|Subscribe successful'
```
